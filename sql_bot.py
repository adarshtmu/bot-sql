...
def simulate_query_duckdb(sql_query, tables_dict):
    ...
    # [simulate_query_duckdb function remains the same]
...
def get_table_schema(table_name, tables_dict):
    ...
    # [get_table_schema function remains the same]
...
def evaluate_answer_with_llm(question_data, student_answer, original_tables_dict):
    ...
    # [evaluate_answer_with_llm function remains the same]
...
def calculate_score(user_answers):
    ...
    # [calculate_score function remains the same]
...
def analyze_performance(user_answers):
    ...
    try:
        ...
        Task: Ab ek dost ki tarah, casual Hindi mein overall performance ka ek summary feedback do... [Rest of prompt] ..."""
...
def display_simulation(title, result_data):
    ...
    # [display_simulation function remains the same]
    st.write(f"**{title}:**")
    if isinstance(result_data, pd.DataFrame):
        if result_data.empty: st.info("_(Simulation resulted in an empty table)_")
        else: st.dataframe(result_data.reset_index(drop=True), hide_index=True)
...
# --- Quiz Completed Screen ---
    ...
    st.metric(label="Your Final Score (%)", value=score) # Corrected metric call
    ...
    if st.session_state.show_detailed_feedback:
        ...
        # Access keys from the returned dictionary using .get for safety
        ...
        if strengths:
            # Use a standard for loop for strengths
        ...
        if weaknesses:
            # Use a standard for loop for weaknesses
